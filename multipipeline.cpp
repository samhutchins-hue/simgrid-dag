...
XBT_LOG_NEW_DEFAULT_CATEGORY(dag_scheduling,
                             "Logging specific to this example");
namespace sg4 = simgrid::s4u;

static std::vector<sg4::Exec *>
get_ready_tasks(const std::vector<sg4::ActivityPtr> &dax) {
  std::vector<sg4::Exec *> ready_tasks;
  std::map<sg4::Exec *, unsigned int> candidate_execs;

  for (const auto &a : dax) {
    // Only look at activity that have their dependencies solved but are not
    // assigned
    if (a->dependencies_solved() && not a->is_assigned()) {
      ...
      // if it a comm, we consider its successor as a candidate. If a candidate
      // solves all its dependencies, i.e., get all its input data, it's ready
      if (const auto *comm = dynamic_cast<sg4::Comm *>(a.get())) {
        auto *next_exec =
            static_cast<sg4::Exec *>(comm->get_successors().front().get());
  ...
  for (const auto &host : sg4::Engine::get_instance()->get_all_hosts()) {
    ...
    for (const auto &parent : exec->get_dependencies()) {
      ...
      if (const auto *comm = dynamic_cast<sg4::Comm *>(parent.get())) {
        ...
        XBT_DEBUG("transfer from %s to %s", source->get_cname(),
                  host->get_cname());
        ...
        // We use the user data field to store the finish time of the
        // predecessor of the comm, i.e., its potential start time
    ...
    double finish_time =
        std::max(*host->get_data<double>(), last_data_available) +
        exec->get_remaining() / host->get_speed();

    XBT_DEBUG("%s finishes on %s at %f", exec->get_cname(), host->get_cname(),
              finish_time);
...
int main(int argc, char **argv) {
  ...
  sg4::Exec::on_completion_cb([](sg4::Exec const &exec) {
    // when an Exec completes, we need to set the potential start time of all
    // its ouput comms
    for (const auto &succ : exec.get_successors()) {
      auto *comm = dynamic_cast<sg4::Comm *>(succ.get());
      if (comm != nullptr) {
  ...
  /* Mark all hosts as sequential, as it ought to be in such a scheduling
   * example.
   *
   * It means that the hosts can only compute one thing at a given time. If an
   * execution already takes place on a given host, any subsequently started
   * execution will be queued until after the first execution terminates */
  ...
  while (not vetoed.empty()) {
    ...
    XBT_INFO("Schedule %s on %s", selected_task->get_cname(),
             selected_host->get_cname());
