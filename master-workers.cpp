...
static void master(std::vector<std::string> args) {
  ...
  XBT_INFO("Got %zu workers and %ld tasks to process", workers.size(),
           tasks_count);
  ...
  for (int i = 0; i < tasks_count; i++) { /* For each task to be executed: */
    ...
    XBT_INFO("Sending task %d of %ld to mailbox '%s'", i, tasks_count,
             mailbox->get_cname());
...
static void worker(std::vector<std::string> args) {
  ...
  simgrid::s4u::Mailbox *mailbox =
      simgrid::s4u::Mailbox::by_name(my_host->get_name());
